...
**Bypass found & FIXED (2026-08-09):** Password change alone didn't stop Benedict — on both his own laptop and Eric's Windows laptop, he got a fully unwrapped Firefox with no session token ever being written, meaning he wasn't going through the password dialog at all. Two confirmed bypass vectors on Windows:
1. **`HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\firefox.exe`** registry key pointed at the real `firefox.exe`. Typing "firefox" into Windows Search or Run (Win+R) launched it directly, no wrapper involved. **Fix:** delete the key (`Remove-Item "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\firefox.exe" -Force`, needs admin). Confirmed gone on both machines.
2. **Firefox "Web App" shortcuts** (e.g. `...\Start Menu\Programs\Firefox Web Apps\Google Mail.lnk`, created automatically by Firefox's "Install this site as an app" feature) also launched `firefox.exe` directly with a full, unrestricted window. Eric's laptop had more than one of these across profiles. **Fix:** rewrite each shortcut's `TargetPath`/`Arguments` in place to route through `powershell.exe -File browser-unlock.ps1 <original args>` instead of `firefox.exe` directly — preserves the exact web-app launch behavior, just password-gated. Confirmed zero remaining direct-to-exe `.lnk` files on both machines via a full recursive scan of `C:\Users` and the Start Menu.

These existed independently of the taskbar/Start Menu/Public Desktop shortcuts that were already wrapped — a shortcut-swap approach can never assume it's complete, since Windows offers many paths to the real binary. **When deploying or re-auditing this lock on any Windows machine, always also check:** the App Paths registry key, and a full recursive `.lnk` scan (not just the 3 known shortcut spots) — see [[feedback_shortcut_lock_incomplete]].
