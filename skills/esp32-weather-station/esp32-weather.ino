...
<body>
  ...
  function saveKids(){fetch('http://'+thinkcentreHost()+':8181/kids/choredone',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kids:kidsData.map(function(k){return{name:k.name,choreDone:k.choreDone||{}};}),weekStart:kidsWeekStart})}).catch(function(){});}
  ...
  function camBase(){var h=window.location.hostname;if(h.match(/^192\.168\./))return{video:'http://192.168.12.211:8080/',audio:'http://192.168.12.211:8081/'};return{video:'http://100.70.179.60:8241/',audio:'http://100.70.179.60:8242/'};}
  function showCam(){var u=camBase();document.getElementById('camOverlay').className='overlay open';document.getElementById('camStream').src=u.video;var a=document.getElementById('camAudio');a.src=u.audio;a.load();a.play();}
