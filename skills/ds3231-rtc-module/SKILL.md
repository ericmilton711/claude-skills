...
### 4. Remove fake-hwclock (after reboot)
```bash
...
### 6. Configure system to read RTC on boot
Edit `/lib/udev/hwclock-set` — comment out the lines that check for systemd:
```bash
...
## Order Info
- Amazon: AITRIP 3-pack or Dorhea 4-pack DS3231, ~$8
- CR2032 coin cell battery (not always included)
