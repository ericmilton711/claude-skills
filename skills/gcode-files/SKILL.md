# G-code Files Inventory

Eric's 3D printing G-code file collection, stored in `~/Desktop/gcode files/`. Printer: **Creality Ender 3 Pro** (files prefixed `CE3PRO_` are sliced specifically for it).
...
**Source files subfolder:** `XP-53 Peregrine (RC Plane)/`
- `first set/` — STP source files + reference images
- `second set/` — STL + STP files, updated parts
- `third set/` — STL files, final revisions
